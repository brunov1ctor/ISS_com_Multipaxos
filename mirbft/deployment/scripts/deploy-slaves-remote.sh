...
#        [trigger0 n0 label0 [trigger1 n1 label1 [...]]]
#
# Deploys slave machines based on master status.
# Uses slave machine addresses provided in instance_info_file instead of starting new ones.
# For each tuple (trigger, n, label) given as arguments on the command line,
# deploys n slave nodes with the given label when the master status is trigger
# or (numerically) higher.
# The input tuples are processed in the given order, so a later tuple is only
# applied when all previous tuples have received their triggers.
...
# Each new invocation of start-remote-slaves.sh will receive this list as the
# last arguments and skip correspondingly many entries in the input.
