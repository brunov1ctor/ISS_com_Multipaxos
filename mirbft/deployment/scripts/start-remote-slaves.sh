...
master_ip=$4
shift 4         # resto são os campos de scripts/instance-info "achatados" em argumentos
...
echo "==== [start-remote-slaves] Verificando binários locais em $local_bin_dir para tag '$tag' ===="
...
#    - Usa os argumentos já passados pelo deploy-remote.sh
#      (instance_id public_ip private_ip role slave_tag) em blocos de 5
###############################################################################

echo "==== [start-remote-slaves] Verificando / copiando binários remotos para tag '$tag' ===="

# Guardar a lista de argumentos (cada 5 itens = 1 linha de instance-info)
args=( "$@" )
total_args=${#args[@]}
idx=0
...
while [ $idx -lt $total_args ]; do
  ...
  public_ip=${args[$((idx+1))]}
  private_ip=${args[$((idx+2))]}
  role=${args[$((idx+3))]}
  slave_tag=${args[$((idx+4))]}

  # Só nos interessam nós com a tag correspondente (ex.: peers ou 1client)
  if [ "$slave_tag" = "$tag" ]; then
    echo "  -> Nó $instance_id ($public_ip) [tag=$slave_tag]: garantindo binários..."

    # Garante diretório remoto /users/Bruno/go/bin
    ssh $ssh_options "$public_ip" "mkdir -p \"$remote_gopath/bin\"" || {
      echo "     [ERRO] Não foi possível criar $remote_gopath/bin em $public_ip" >&2
    }

    # Para cada binário, verifica se existe; se não, copia
    for b in $binaries; do
      ssh $ssh_options "$public_ip" "[ -x \"$remote_gopath/bin/$b\" ]"
      if [ $? -eq 0 ]; then
        echo "     - $b já existe em $public_ip:$remote_gopath/bin/$b"
      else
        echo "     - $b AUSENTE em $public_ip. Copiando de $local_bin_dir/$b..."
        if [ ! -x "$local_bin_dir/$b" ]; then
          echo "       [ERRO] Binário local $local_bin_dir/$b não existe; não há o que copiar." >&2
        else
          scp "$local_bin_dir/$b" "$public_ip:$remote_gopath/bin/" || {
...
###############################################################################
# 3) Lógica de 'skip' e início de discoveryslave nos nós remotos
#    (usa novamente os argumentos, mas agora via $1..$N)
###############################################################################

# Reconstituir a lista original de argumentos de instância (porque já usamos args[])
set -- "${args[@]}"
...
# Para cada linha de instance-info passada como argumentos:
